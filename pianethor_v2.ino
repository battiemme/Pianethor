...
unsigned long thresholdTimeout = 1000; // Timeout in milliseconds before detecting next threshold cross
...
void setupWebServer() {
  ...
  server.on("/setSpeed", HTTP_POST, []() {
    if (server.hasArg("speed")) {
      ...
      // Metti in pausa automaticamente quando cambi velocità
  ...
  server.on("/analyzeFile", HTTP_GET, []() {
    // Se non hai nulla salvato ma hai un file temp, rianalizza
  ...
  server.on("/selectTrack", HTTP_POST, []() {
    if (server.hasArg("trackIndex") && server.hasArg("trackSlot") && hasAnalyzedTracks) {
      ...
      // Check if total notes would exceed limit
      int totalNotesAfter = trackNoteCount;
      if (trackSlot == 2) {
        totalNotesAfter += notes.size();
      }
      
      if (totalNotesAfter > safeNotes) {
        String errorMsg = "Too many notes : the maximum is limited to " + String(safeNotes);
        if (debugMode) Serial.println(errorMsg);
        server.send(400, "text/plain", errorMsg);
        return;
      }
...
void playMusic() {
  ...
  // Calcola il tempo REALE passato, NON moltiplicato per playbackSpeed
  unsigned long elapsedTime = (now - playStartTime - totalPauseTime);
  
  // Se microphone è abilitato e single track: ONLY advance by volume threshold
  if (microphoneEnabled && selectedTrackIndex2 == -1) {
    static unsigned long lastDetectionAttempt = 0;
    static bool firstNoteShown = false;
    ...
    if (!firstNoteShown && currentNoteIndex < notes.size()) {
      ...
      while (currentNoteIndex < notes.size() && notes[currentNoteIndex].startTime == noteTime) {
        ...
        if (note.ledIndex >= 0 && note.ledIndex < NUM_LEDS) {
          ...
          ledEndTimes[note.ledIndex] = ULONG_MAX;  // Never turn off
        ...
        if (currentNoteIndex < notes.size()) {
          ...
          while (currentNoteIndex < notes.size() && notes[currentNoteIndex].startTime == noteTime) {
            ...
            if (note.ledIndex >= 0 && note.ledIndex < NUM_LEDS) {
              ...
              Serial.printf("Volume detected: %.1f (threshold: %.1f) - Showing note %d\n", volume, volumeThreshold, currentNoteIndex);
            }
  ...
  // If microphone is disabled or dual track: advance by timing with playbackSpeed
  ...
  if (currentNoteIndex < notes.size()) {
    ...
    while (currentNoteIndex < notes.size() && scaledElapsedTime >= notes[currentNoteIndex].startTime) {
      Note& note = notes[currentNoteIndex];
...
String getMainPage() {
  ...
  html += ".loading-content { background: rgba(255, 255, 255, 0.1); padding: 40px; border-radius: 20px; text-align: center; max-width: 400px; }";
  ...
  html += "<div style='text-align: center; margin-top: -20px; margin-bottom: 20px; font-size: 0.9em; opacity: 0.8;'>a free project by battiemme</div>";
  html += "<div style='text-align: center; margin-bottom: 20px;'>";
  html += "<label style='display: flex; align-items: center; justify-content: center; gap: 10px;'>";
  html += "<span style='font-size: 0.9em;'>Debug Mode:</span>";
  ...
  html += "<input type='number' id='thresholdTimeout' min='100' step='50' value='1000' style='width: 100px;'>";
  ...
  html += "};";
  html += "xhr.send('enabled=' + (enabled ? 'true' : 'false'));";
  html += "var statusXhr = new XMLHttpRequest();";
  html += "statusXhr.onload = function() {";
  html += "if (statusXhr.status === 200) {";
  html += "var data = JSON.parse(statusXhr.responseText);";
  html += "if (data.isPlaying) {";
  html += "stop();";
  html += "}";
  html += "}";
  html += "};";
  html += "statusXhr.open('GET', '/status');";
  html += "statusXhr.send();";
  html += "};";
  ...
  html += "document.getElementById('thresholdTimeout').value = 1000;";
